    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          const double latency = 0.1; // (100 / 1000)
          const long actuator_latency = 100; // in milliseconds
          ...
          double psi_lat = -(v / Lf) * delta * latency;
          double v_lat = v + acceleration * latency;

          Eigen::VectorXd state(6);
          state << x_lat, 0, psi_lat, v_lat, cte, epsi;
